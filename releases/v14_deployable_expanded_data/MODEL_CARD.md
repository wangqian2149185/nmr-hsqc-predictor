...
## Deployable release freeze

This standalone snapshot was frozen in STEP 59C from the independently clean-reload-verified candidate.

- Release status: deployable_release_frozen
- Five-seed ensemble checkpoints changed during freeze: no
- Preprocessing values changed during freeze: no
- Inference implementation changed during freeze: no
- Dataset targets or splits changed during freeze: no
- External-blind cohort opened during freeze: no
- Reference correction applied: no
- Hierarchical b_j used: no

The release is eligible for one final external-blind evaluation. External results are reporting-only and must not be used to modify this frozen model.
